...
float modelRotX = 90.0f, modelRotY = -140.0f, modelRotZ = 100.0f; // Model rotation
...
void keyboard(unsigned char key, int x, int y) {
   float moveAmount = 10.0f; // Light movement speed
      ...
      switch (key) {
      case 'w': modelRotX += moveAmount; break; // Move light forward
      case 's': modelRotX -= moveAmount; break; // Move light backward
      case 'a': modelRotY -= moveAmount; break; // Move light left
      case 'd': modelRotY += moveAmount; break; // Move light right
      case 'q': modelRotZ += moveAmount; break; // Move light up
      case 'e': modelRotZ -= moveAmount; break; // Move light down
      }
   
      std::cout <<  modelRotX << ", " <<  modelRotY << ", " <<  modelRotZ << ", " << std::endl;
...
void display() {
   ...
   gluLookAt(10.0f, 10.0f, 0.0f,  // Camera position
...
void init() {
   ...
   glLoadIdentity();
   glOrtho(-100.0, 100.0, -100.0, 100.0, -100.0, 1000.0); // Orthographic projection setup
   // Load the model
...
void reshape(int w, int h) {
   ...
   glMatrixMode(GL_PROJECTION);
   glLoadIdentity();
   glOrtho(-100.0, 100.0, -100.0, 100.0, -100.0, 1000.0);
...
int main(int argc, char** argv) {
   ...
   glutCreateWindow("Task1 - Orthographic Projection");
